...
import { Check, CheckCircle2, CalendarX2 } from "lucide-react"
...
export function TeacherApprovalList({ results }: { results: PendingResult[] }) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-slate-800">Pending Teacher Approvals</h2>
                    <p className="text-slate-500 text-sm">Review your students' certifications.</p>
                </div>
            ...
            {filteredResults.length === 0 ? (
                <div className="text-center py-20 bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-200">
                    ...
                    <p className="text-slate-400 max-w-xs mx-auto mt-2">
                        {selectedDate
                            ? `There are no pending certifications for ${format(selectedDate, "PPP")}.`
                            : "All pending certifications have been processed."}
                    </p>
                    {selectedDate && (
                        <Button
                            variant="link"
                        ...
                        <Card key={result.id} className="group overflow-hidden border-none shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 rounded-3xl bg-white">
                            ...
                            <CardContent className="pt-4">
                                ...
                                <Button
                                    ...
                                    className="w-full h-12 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-lg shadow-indigo-100 transition-all active:scale-[0.98]"
                                >
                                    <Check className="mr-2 h-5 w-5" /> Approve Certificate
                                </Button>
                            </CardContent>
                        </Card>
                    ))}
            ...
            {results.every(r => approvedIds.has(r.id)) && filteredResults.length > 0 && (
                <div className="text-center py-16 bg-blue-50/50 rounded-3xl border-2 border-dashed border-blue-200">
                    <CheckCircle2 className="h-16 w-16 text-blue-500 mx-auto mb-4 drop-shadow-sm" />
                    <h3 className="text-2xl font-bold text-blue-800">Excellent!</h3>
                    <p className="text-blue-600 font-medium">All student certificates have been approved.</p>
                </div>
            )}
        </div>
    )
